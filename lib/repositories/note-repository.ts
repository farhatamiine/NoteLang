...
interface NoteRepository {
    ...
    generateWordExample(noteId: string, input: AiInput, userId: string): Promise<Result<GeneratedExamplesPayload>>
}
...
export class SupabaseNoteRepository implements NoteRepository {
    ...
    async generateWordExample(noteId: string, input: AiInput, userId: string): Promise<Result<GeneratedExamplesPayload>> {
        ...
        if (cached && Date.now() - new Date(cached.created_at).getTime() < FRESH_MS) {
            return {success: true, data: cached.output_json as GeneratedExamplesPayload};
        }
        const output = (await generateWordExamples(normalized)) as GeneratedExamplesPayload;
        if (!output?.examples?.length) return {success: false, error: "AI returned no examples."};
        ...
        const {data: setRow, error: setErr} = await this.db
            ...
            .insert({
                ...
                output_json: output,
                model_id: output.meta.model ?? "gemini-2.5-flash"
            })
            .select("id")
            .single();
        if (setErr || !setRow) return {success: false, error: setErr?.message ?? "Failed to save example set."};
